...
os.makedirs("output-YCbCr", exist_ok=True)
...
# HISTOGRAM EQUALIZATION pada channel Y di ruang YCbCr
# -------------------------------------------------------
# Perubahan dari versi sebelumnya:
#   SEBELUM : HEQ diterapkan per-channel (B, G, R) secara independen
#             → menyebabkan color shift karena relasi antar-channel berubah
#   SESUDAH : Gambar dikonversi ke YCbCr, HEQ hanya diterapkan pada
#             channel Y (luminance/kecerahan), lalu dikonversi kembali
#             ke BGR → warna asli tetap terjaga karena Cb dan Cr tidak diubah
# -------------------------------------------------------
def histogram_equalization_ycbcr(img_bgr):
    """
    Terapkan HEQ hanya pada channel Y (luminance) di ruang warna YCbCr.
    Channel Cb dan Cr (chrominance/warna) tidak disentuh sehingga
    tidak terjadi pergeseran warna (color shift).

    Parameter
    ---------
    img_bgr : np.ndarray
        Gambar BGR uint8 (output dari Gaussian Filter, sudah 3-channel).

    Return
    ------
    np.ndarray  BGR uint8 dengan luminance yang sudah di-equalize.
    """
    # Konversi BGR -> YCbCr
    ycbcr = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2YCrCb)
    Y, Cr, Cb = cv2.split(ycbcr)

    # HEQ hanya pada channel Y (sama persis logikanya dengan fungsi lama)
    hist = np.zeros(256)
    for pixel in Y.flatten():
        hist[pixel] += 1
    cdf = np.cumsum(hist)
    ...
    # Gabungkan kembali dan konversi YCbCr -> BGR
...
def plot_pipeline_images(images_dict, save_path="output-YCbCr/pipeline_comparison.png"):
...
def plot_histogram_comparison(images_dict, save_path="output-YCbCr/histogram_comparison.png"):
...
def plot_cdf_comparison(before_bgr, after_bgr,
                        save_path="output-YCbCr/cdf_comparison.png"):
    ch_info = [("Blue", 0, "#3a7ebf"),
               ("Green", 1, "#3aab5c"),
               ("Red",   2, "#d94f3d")]

    fig, axes = plt.subplots(1, 3, figsize=(13, 4))
    fig.suptitle("CDF: Sebelum vs Sesudah Histogram Equalization (YCbCr)",
...
def main():
    ...
    cv2.imwrite("output-YCbCr/1_median.png", median_img)
    ...
    cv2.imwrite("output-YCbCr/2_gaussian.png", gaussian_img)

    # Step 3: Histogram Equalization via YCbCr (hanya channel Y)
    # gaussian_img sudah berupa gambar BGR 3-channel — langsung diproses
    he_img = histogram_equalization_ycbcr(gaussian_img)
    cv2.imwrite("output-YCbCr/3_histogram_equalization.png", he_img)

    # Step 4: Unsharp Masking (Sharpening)
    # Unsharp mask diterapkan per-channel seperti semula,
    # tapi sekarang inputnya sudah tidak color-shifted
    b_he, g_he, r_he = cv2.split(he_img)
    b_sharp = unsharp_mask(b_he)
    g_sharp = unsharp_mask(g_he)
    r_sharp = unsharp_mask(r_he)
    final_img = cv2.merge([b_sharp, g_sharp, r_sharp])
    cv2.imwrite("output-YCbCr/4_sharpening.png", final_img)
    cv2.imwrite("output-YCbCr/lena_restored.png", final_img)
